...
| core/postflight/data/qc_report.json | N/A | No objective defined. |
...
| core/dashboard/dashboard.py | 4.3.1 | Dynamic 6-character Maidenhead injection and refined telemetry caching. |
...
| core/preflight/hardware_audit.py | 1.3.0 | Deep hardware audit using the get_event_state bus, exporting to hardware_telemetry.json. |
...
| tests/ch-cyg.json | N/A | Static test payload for pipeline validation. |
...
| tests/star_tour.json | N/A | Static test payload for pipeline validation. |
...
| utils/generate_manifest.py | 1.3.1 | Generate a comprehensive FILE_MANIFEST.md including recursive directory walking, JSON metadata extraction, and ignoring __init__.py. |
...
| data/hardware_telemetry.json | N/A | No objective defined. |
| data/ledger.json | N/A | Master Observational Register and Status Ledger |
| data/ssc_payload.json | N/A | Compiled native SSC JSON payload for Seestar execution. |
| data/system_state.json | N/A | System state initialization to safe IDLE before flight operations. |
| data/tonights_plan.json | N/A | Tactical flight plan filtered by Ledger Cadence. |
| data/weather_state.json | N/A | No objective defined. |
