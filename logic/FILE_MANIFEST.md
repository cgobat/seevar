...
| core/preflight/aavso_fetcher.py | 12.0.0 | Step 1 - Haul AAVSO targets and strictly filter by 30-degree horizon physics. |
...
| core/preflight/ledger_manager.py | 2.1.0 | The High-Authority Mission Brain. Manages target cadence and observation history to determine if a new observation is in order. |
| core/preflight/librarian.py | 4.2.0 | The Single Source of Truth. Manages metadata, purges corruption, |
| core/preflight/nightly_planner.py | 2.5.0 | Executes the 6-step filtering funnel using the Federated Catalog. |
...
| core/preflight/state_flusher.py | 1.0.0 | Preflight utility to flush stale system state and reset the dashboard to IDLE before a new flight. |
...
| core/flight/orchestrator.py | 2.1.0 | Upload the mission JSON and trigger the state machine from a verified Zero-State. |
...
| core/postflight/science_processor.py | 2.1.0 | Automate Siril Green-channel extraction using native siril-cli. |
